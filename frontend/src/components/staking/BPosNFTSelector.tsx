...
const BPosNFTEntry: FC<{
  ...
  return <div className="flex flex-row items-center gap-2 my-2">
    <Checkbox id={nft.id} onCheckedChange={onSelectionChanged} disabled={nft.voteInfo?.getCoinValue() === 0} />
    <div className="flex flex-col">
      <label htmlFor={nft.id} className="text-xs">
        BPoS NFT #{formatAddress(nft.tokenId, [8, 8])}
        <span className="text-primary"> {nft.voteInfo?.getCoinValue()} {activeChain.nativeCurrency.symbol} votes</span>
      </label>
    </div>
  </div>
}