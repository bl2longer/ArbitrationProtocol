 ...
 *
 * NOTE: Later when adding more wallet support such as wallet connect we have to store the connector in use, and try to auto reconnect
...
const AutoReconnect: FC<{ children: ReactNode }> = ({ children }) => {
  const { connect: handleConnect } = useEVMContext();
...
export const useWalletContext = (): WalletContextProps => {
  ...
  if (!context)
    throw new Error('useWalletContext must be used within a WalletContext provider');
